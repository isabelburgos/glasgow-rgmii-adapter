# Pinouts and Pin Compatibility
## 40-pin QFNs
- [MxL86110](Datasheets/mxl86110.pdf)
- [YT8531C](Datasheets/YT8531C.pdf)
- [RTL8211F-CG](Datasheets/RTL8211F-CG.pdf)

| Part#  | [MxL86110](Datasheets/mxl86110.pdf) | [YT8531C](Datasheets/YT8531C.pdf) | [RTL8211F-CG](Datasheets/RTL8211F-CG.pdf)
|----|----|----|----|
| Function  | Pin# | Pin# | Pin# |
| TP A+ | 1 | 1 | 1
| TP A- | 2 | 2 | 2
| VA Low | 3 | 3 | 3
| TP B+ | 4 | 4 | 4
| TP B- | 5 | 5 | 5
| TP C+ | 6 | 6 | 6
| TP C- | 7 | 7 | 7
| V Low | 8 | 8 | 8
| TP D+ | 9 | 9 | 9
| TP D- | 10 | 10 | 10
| VA 3.3 | 11 | 11 | 11
| Reset | 12 | 12 | 12
| MDC | 13 | 13 | 13
| MDIO | 14 | 14 | 14
| TXD3 | 15 | 15 | 15
| TXD2 | 16 | 16 | 16
| TXD1 | 17 | 17 | 17
| TXD0 | 18 | 18 | 18
| TX_CTL | 19 | 19 | 19
| TX_CLK | 20 | 20 | 20
|VD Low | 21 | 21 | 21
|RXD3/PHYAD0 | 22 | 22 | 22
|RXD2/PLLOFF | 23 | 23 | 23
|RXD1/TXDLY | 24 | 24 | 24
|RXD0/RXDLY|25 | 25 | 25
|RX_CTL/PHYAD2 | 26 | 26 | 26
|RX_CLK/PHYAD1 |27 | 27 | 27
|MDIO V Supply|28 | 28 | 28
| VD 3.3 | 29 | 29 | 29
|Regulator Supply Out|30 | 30 | 30
|Interrupt / PME | 31 | 31 | 31 | 31
|LED_0/CFG_EXT|32 | 32 | 32 | 32
|LED1/CFG_LDO0|33 | 33 | 33 | 33
|LED2/CFGLDO1|34 | 34 | 34 | 34
|Internal CLK out|35 | 35 | 35 | 35
|XTAL in| 36 | 36 | 36 | 36
|XTAL out or Ext CLK| 37 | 37 | 37
|VA Low| 38 | 38 | 38 |
|R SET|39 | 39 | 39 
|VA 3V3| 40 | 40 | 40




## 48-pin QFNs
- [RTL8211E](Datasheets/RTL8211E(G)-xx-CG.pdf)
- [RTL8211D](Datasheets/RTL8211D-VB-GR.pdf)
- [KSZ9031RNX](Datasheets/KSZ9031RNX.pdf)
    - Die photo: https://siliconpr0n.org/map/micrel/ksz9031rnx/azonenberg_mz_mit20x


| Part#  | [RTL8211E](Datasheets/RTL8211E(G)-xx-CG.pdf) | [RTL8211D](Datasheets/RTL8211D-VB-GR.pdf)
|----|----|---|
| Function  | Pin# |
| BI_DA+ TX / BI_DB+ RX | 1 | 1
| BI_DA- TX / BI_DB- RX | 2 | 2
| VA Low | 3 | 3
| BI_DB+ TX / BI_DA+ RX | 4 | 4
| BI_DB- TX / BI_DA- RX | 5 | 5
| VA High | 6 | 6
| BI_DC+ TX / BI_DD+ RX | 7 | 7
| BI_DC- TX / BI_DD- RX | 8 | 8
| VA Low | 9 | 9
| BI_DD+ TX / BI_DC+ RX | 10 | 10
| BI_DD- TX / BI_DC- RX | 11 | 11
| NC | 12 | 12
| RX_CTL | 13 | 13 |
| PHYAD2 | 13 | - |
| RXD0/Voltage Select | 14 | 14
| VD High | 15  | 15
| RXD1/TXDLY | 16 | 16
| RXD2/AN0 | 17 | 17
| RXD3/AN1 | 18 | 18
| RXC | 19 | 19
| Interrupt | 20 | 20
| VD 3V3 | 21 | 21
| TX_CLK | 22 | 22
| TXD0 | 23 | 23
| TXD1 | 24 | 24
| TXD2 | 25 | 25
| TXD3 | 26 | 26
| TX_CTL | 27 | 27
| VD Low | 28 | 28
| Reset | 29 | 29
| MDC | 30 | 30
| MDIO | 31 | 31
| LED2/RXDLY | 32 | 32
| PME | 33 | 33
| LED0/PHYAD0 | 34 | 34
| LED1/PHYAD1 | 35 | 35
| VD Low | 36 | 36
| VD High | 37 | 37
| EN Switching Reg | 38 | 38
| R SET | 39 | 39
| VA Low | 40 | 40 
| VA High | 41 | 41
| XTAL In | 42 | 42
| XTAL Out | 43 | 43
| Regulator 3V3 Supply | 44 | 44
| Regulator 3V3 Supply | 45 | 45
| 125 MHz Internal Clk | 46 | 46
| GND | 47 | 47
| Regulator Supply Out | 48 | 48


| Part# | [KSZ9031RNX](Datasheets/KSZ9031RNX.pdf) |
|---|---|
|Function| Pin# |
| VA High | 1 
| BI_DA+ TX / BI_DB+ RX | 2
| BI_DA- TX / BI_DB- RX | 3
| VA Low | 4
| BI_DB+ TX / BI_DA+ RX | 5
| BI_DB- TX / BI_DA- RX | 6
| BI_DC+ TX / BI_DD+ RX | 7 
| BI_DC- TX / BI_DD- RX | 8
| VA Low | 9
| BI_DD+ TX / BI_DC+ RX | 10 
| BI_DD- TX / BI_DC- RX | 11 
| VA High | 12
| NC | 13
| VD Low | 14
| LED2/PHYAD1 | 15
| VD High | 16
| LED1/PHYAD0/PME_N1 | 17
| VD Low | 18
| TXD0 | 19
| TXD1 | 20
| TXD2 | 21
| TXD3 | 22
| VD Low | 23
| TX_CLK | 24
| TX_CTL | 25
| VD Low | 26
| RXD3/MODE3 | 27
| RXD2/MODE2 | 28
| GND | 29
| VD Low | 30
| RXD1/MODE1 | 31
| RXDO/MODE0 | 32
| RX_DV/CLK125_EN | 33
| VD High | 34
| RX_CLK/PHYAD2 | 35
| MDC | 36
| MDIO | 37
| INT_N/PME_N2 | 38
| VD Low | 39
| VD High | 40
| 125 MHz Internal CLK/LED_MODE | 41
| Reset | 42
| LDO_O | 43
| VD Low PLL | 44
| Xtal Out | 45
| Xtal In | 46
| NC | 47 |
| I Set | 48

## 56-pin QFNs
- [LAN8820](Datasheets/LAN8820.pdf)


## 64-pin QFNs
- [RTL8211DG](Datasheets/RTL8211D-VB-GR.pdf)

## 88-pin QFNs
- [RTL8211DN](Datasheets/RTL8211D-VB-GR.pdf)

# What interfaces are supported?
| Part# | RGMII | GMII | SGMII
|---|---|---|---|
| [RTL8211E](Datasheets/RTL8211E(G)-xx-CG.pdf) | ✅ | | |
| [RTL8211D](Datasheets/RTL8211D-VB-GR.pdf) | ✅ | | |
| [KSZ9031RNX](Datasheets/KSZ9031RNX.pdf) | ✅ | | |
| [RTL8211EG](Datasheets/RTL8211E(G)-xx-CG.pdf) | ✅ | ✅ | |
| [RTL8211DG](Datasheets/RTL8211D-VB-GR.pdf) | ✅ | ✅ | |
| [RTL8211DN](Datasheets/RTL8211D-VB-GR.pdf) | ✅ | ✅ | ✅ |


## Jumbo Frames
| Part#  | Jumbo Frames Support
|---|---|
[MxL86110](Datasheets/mxl86110.pdf) | 10k bytes
[KSZ9031RNX](Datasheets/KSZ9031RNX.pdf) | 16k bytes


